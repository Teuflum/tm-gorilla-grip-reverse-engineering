...
- On landing it waits at least 30 ms for the grounded physics update, then shows a grade with a plain reason: no direction stored before takeoff, opposite direction stored, insufficient force recovery, no landing steering, or a successful pre-set direction.
...
- A candidate requires icy tires before airtime and a minimum airborne duration. Any wheel contact is the grounded boundary. The stored direction must match the landing direction, and the first settled force reading at least 30 ms after visible contact determines the grade. Icing, airtime, and steering thresholds are configurable filters, not claims of universal slide requirements.
